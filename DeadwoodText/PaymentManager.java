...
public class PaymentManager {
    ...
    public void payBonuses(Room room) {
        ...
        if (playersOnCard.size() > 0) {
            ...
            for (int i = budgetRoll.length-1; i >= 0; i--) {
                ...
                System.out.println("\t$$$ Player " + playersOnCard.get(pos).getID() + "(r: " + playersOnCard.get(pos).getRole().getRank() + ") awarded " + budgetRoll[i] + " dollars $$$");
            ...
            if (playersOffCard.size() > 0) {
                ...
                for (Player p : playersOffCard) {
                    ...
                    System.out.println("\t$$$ Player " + p.getID() + " awarded " + p.getRole().getRank() + " dollars $$$");
                }
    ...
    public void payActAction(Player player, boolean success) {
        ...
        if (role.isExtra()) {
            if (success) {
                System.out.println("$$$ Player " + player.getID() + "(r: " + player.getRole().getRank() + ") awarded 1 credit and 1 dollar $$$");
                player.addCredits(1);
                player.addDollars(1);
            } else player.addDollars(1);
        } else {
            if (success) {
                System.out.println("$$$ Player " + player.getID() + "(r: " + player.getRole().getRank() + ") awarded 2 credits $$$");
                player.addCredits(2);
            }
        }
    }
